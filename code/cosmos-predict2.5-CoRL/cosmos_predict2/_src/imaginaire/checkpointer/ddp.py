...
class Checkpointer(AbstractCheckpointer):
    ...
    def _save_worker(self, state_dict: dict[str, StateDictItemPath], checkpoint_file: str, rank: int = 0) -> None:
        ...
        try:
            ...
            self._write_epoch_aliases(checkpoint_file, iteration)
    ...
    def _write_epoch_aliases(self, checkpoint_file: str, iteration: int) -> None:
        """Create optional human-readable model checkpoint aliases for local training jobs."""
        if distributed.get_rank() != 0 or self.save_to_object_store:
            return
        epoch_step = int(os.environ.get("COSMOS_EPOCH_CKPT_STEP", "0") or 0)
        final_step = int(os.environ.get("COSMOS_FINAL_CKPT_STEP", "0") or 0)
        aliases = []
        if epoch_step > 0 and iteration % epoch_step == 0:
            aliases.append(f"checkpoint-epoch{iteration // epoch_step}-step{iteration}.pt")
        if final_step > 0 and iteration == final_step:
            aliases.append(f"checkpoint-final-step{iteration}.pt")
        if not aliases:
            return

        model_file = checkpoint_file.replace(".pt", "_model.pt")
        model_path = os.path.join(self.save_dirname, model_file)
        if not os.path.exists(model_path):
            self.print(f"Skip checkpoint alias; model file not found: {model_path}")
            return
        for alias in aliases:
            alias_path = os.path.join(self.save_dirname, alias)
            try:
                if os.path.lexists(alias_path):
                    os.unlink(alias_path)
                os.link(model_path, alias_path)
                self.print(f"Created checkpoint alias {alias_path} -> {model_path}")
            except OSError:
                import shutil

                shutil.copy2(model_path, alias_path)
                self.print(f"Copied checkpoint alias {alias_path} from {model_path}")
    ...
    def generate_save_state_dict(
        ...
        if self.rank_dp_w_cp == 0:
            save_model_only = os.environ.get("COSMOS_SAVE_MODEL_ONLY", "0").lower() in {"1", "true", "yes"}
            trainer_state = dict(
            ...
            trainer_state, model_state = misc.to([trainer_state, model_state], device="cpu")
            state_dict = {"model": model_state}
            if not save_model_only:
                optim_state = optimizer.state_dict()
                scheduler_state = scheduler.state_dict()
                optim_state, scheduler_state = misc.to([optim_state, scheduler_state], device="cpu")
                state_dict.update(
                    {
                        "optim": optim_state,

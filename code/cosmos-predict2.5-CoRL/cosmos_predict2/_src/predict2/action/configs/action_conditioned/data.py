...
try:
    from cosmos_predict2._src.predict2.action.configs.action_conditioned.experiment.exp_2B_afb_delta_ee_pcp_pob import (
        register_afb_delta_ee_pcp_pob_data,
    )
except ImportError:
    register_afb_delta_ee_pcp_pob_data = None

try:
    from cosmos_predict2._src.predict2.action.configs.action_conditioned.experiment.exp_2B_afb_delta_ee import (
        register_afb_delta_ee_data,
    )
except ImportError:
    register_afb_delta_ee_data = None

try:
    from cosmos_predict2._src.predict2.action.configs.action_conditioned.experiment.exp_2B_afb_s1_family_balanced import (
        register_afb_s1_family_balanced_data,
    )
except ImportError:
    register_afb_s1_family_balanced_data = None

try:
    from cosmos_predict2._src.predict2.action.configs.action_conditioned.experiment.exp_2B_afb_s1_expert_single_task import (
        register_afb_s1_expert_single_task_data,
    )
except ImportError:
    register_afb_s1_expert_single_task_data = None

try:
    from cosmos_predict2._src.predict2.action.configs.action_conditioned.experiment.exp_2B_robotwin_clean50_all50 import (
        register_robotwin_all50_data,
    )
except ImportError:
    register_robotwin_all50_data = None

try:
    from cosmos_predict2._src.predict2.action.configs.action_conditioned.experiment.exp_2B_robotwin_clean50 import (
        register_robotwin_clean50_data,
    )
except ImportError:
    register_robotwin_clean50_data = None
...
def register_training_and_val_data():
    ...
    if register_afb_delta_ee_pcp_pob_data is not None:
        register_afb_delta_ee_pcp_pob_data()

    # Register afb_delta_ee data
    if register_afb_delta_ee_data is not None:
        register_afb_delta_ee_data()

    # Register AFB S1 family-balanced data
    if register_afb_s1_family_balanced_data is not None:
        register_afb_s1_family_balanced_data()

    # Register AFB S1 single-task expert-only data
    if register_afb_s1_expert_single_task_data is not None:
        register_afb_s1_expert_single_task_data()

    # Register robotwin all50 data
    if register_robotwin_all50_data is not None:
        register_robotwin_all50_data()

    # Register robotwin clean50 data
    if register_robotwin_clean50_data is not None:
        register_robotwin_clean50_data()

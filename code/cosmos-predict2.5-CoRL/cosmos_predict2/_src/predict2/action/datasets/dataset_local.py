...
class Dataset_3D(Dataset):
    def __init__(
        ...
        self.action_dim = 14  # left: xyz(3)+euler(3)+gripper(1), right: xyz(3)+euler(3)+gripper(1)
        self.c_act_scaler = [20.0, 20.0, 20.0, 20.0, 20.0, 20.0, gripper_rescale_factor,
                             20.0, 20.0, 20.0, 20.0, 20.0, 20.0, gripper_rescale_factor]
    ...
    def _get_robot_states(self, label, frame_ids):
        ...
        # states: (T, 14) = left xyz+euler(6) + left_gripper(1) + right xyz+euler(6) + right_gripper(1)
        # arm_states: (T, 12) = left xyz+euler(6) + right xyz+euler(6)
        # gripper_states: (T, 2) = left_gripper + right_gripper
        arm_states = np.concatenate([states[:, :6], states[:, 7:13]], axis=1)  # (T, 12)
        gripper_states = np.stack([states[:, 6], states[:, 13]], axis=1)        # (T, 2)
    ...
    def _get_actions(self, arm_states, gripper_states, accumulate_action):
        ...
        left_arm  = arm_states[:, :6]
        right_arm = arm_states[:, 6:]
        if accumulate_action:
            base_l_xyz, base_l_rpy = left_arm[0, :3], left_arm[0, 3:]
            base_r_xyz, base_r_rpy = right_arm[0, :3], right_arm[0, 3:]
            base_l_rotm = euler2rotm(base_l_rpy)
            base_r_rotm = euler2rotm(base_r_rpy)
            for k in range(1, self.sequence_length):
                rel_xyz_l = np.dot(base_l_rotm.T, left_arm[k, :3] - base_l_xyz)
                rel_rpy_l = rotm2euler(base_l_rotm.T @ euler2rotm(left_arm[k, 3:]))
                rel_xyz_r = np.dot(base_r_rotm.T, right_arm[k, :3] - base_r_xyz)
                rel_rpy_r = rotm2euler(base_r_rotm.T @ euler2rotm(right_arm[k, 3:]))
                action[k - 1, :3]  = rel_xyz_l
                action[k - 1, 3:6] = rel_rpy_l
                action[k - 1, 6]   = gripper_states[k, 0]
                action[k - 1, 7:10] = rel_xyz_r
                action[k - 1, 10:13] = rel_rpy_r
                action[k - 1, 13]  = gripper_states[k, 1]
                if k % 4 == 0:
                    base_l_xyz, base_l_rpy = left_arm[k, :3], left_arm[k, 3:]
                    base_r_xyz, base_r_rpy = right_arm[k, :3], right_arm[k, 3:]
                    base_l_rotm = euler2rotm(base_l_rpy)
                    base_r_rotm = euler2rotm(base_r_rpy)
        else:
            for k in range(1, self.sequence_length):
                prev_l_rotm = euler2rotm(left_arm[k - 1, 3:])
                rel_xyz_l = np.dot(prev_l_rotm.T, left_arm[k, :3] - left_arm[k - 1, :3])
                rel_rpy_l = rotm2euler(prev_l_rotm.T @ euler2rotm(left_arm[k, 3:]))
                prev_r_rotm = euler2rotm(right_arm[k - 1, 3:])
                rel_xyz_r = np.dot(prev_r_rotm.T, right_arm[k, :3] - right_arm[k - 1, :3])
                ...
                action[k - 1, 10:13] = rel_rpy_r
                action[k - 1, 13]   = gripper_states[k, 1]
        return torch.from_numpy(action)  # (T-1, 14)
